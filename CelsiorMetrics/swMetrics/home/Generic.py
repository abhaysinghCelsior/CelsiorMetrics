...
from swMetrics.home.models import AutomatedUnitTestingEffectiveness,EffortVariance,TmpPswd,DefectSource,RC,DefectSummary
...
def ValidateDefectSummaryInputs(DefectId, DefectSrc_I,RC_I,request):
        ...
        for valDefId in DefectId1:
                if((valDefId=='DefectId') or (len(valDefId.strip())==0)):
                        messages.success(request, "One of defect Number is not Given")
                        return "E"
                if(last==valDefId):
                        messages.success(request, "Defect Numbers should be Unique")
                        return "E"
                last=valDefId
        for valDefSrc in DefectSrc_I:
                if(valDefSrc=="None"):
                        messages.success(request, "Defect Source is not selected for one of the Defect")
                        return "E"
        for valRC in RC_I:
                if(valRC=="None"):
                        messages.success(request, "Route Cause is not selected for one of the Defect")
                        return "E"
        return "S"
...
def GEN_MSG(upd,request,MET,FIELD1,FIELD2):
        if(upd=='C'):
                ...
                messages.success(request, "Both Fields Only Accept Numeric Value")
        elif(upd=='N'):
                messages.success(request, "Fields Should Not Be Null")
        elif(upd=='Z'):
                messages.success(request, FIELD1 + " are  Zero While " + FIELD2 +  "More Than 0")
        elif(upd=='F'):
                messages.success(request, FIELD1 + " and " + FIELD2 +  " both can't not be 0")
        elif(upd=='E'):
                messages.success(request, FIELD1 + " Can't Be More Than " + FIELD2)
        elif(upd=='B'):
                ...
                messages.success(request, "Only 0 to 100 Acceptable")
        else:
...
def dispReportMsg(ERR,PPT_Name,request):
        ...
        if ERR in SUCC:
                ...
                messages.success(request, "TO should be greater than FROM")
        elif (ERR=='E2'):
                messages.success(request, "TO is not Selected")
        elif (ERR=='E3'):
                messages.success(request, "From is not Selected")
        elif (ERR=='E4'):
                messages.success(request, "No option Selected")
        else:
...
def saveBenchMark(MONTH,MET,BNCHMARK):
        try:
                ...
                MetBnhMrkOld=None
        print("Month:"+MONTH+" Metrics:"+MET+" BenchMark:"+str(BNCHMARK))
...
"""
    ...
    fernet = Fernet(key)
    return fernet.encrypt(mypwd.encode())
"""
...
def getPRJ(MetObj,PRJ_KEY,Month):
        try:
...
def genListProjDidntAddedMetricsData():
        ...
        if(MetMonth):
                ...
                for AppObj in AppMetricsList:
                        MetObj=OBJ[AppObj.Metrics]
                        PRJ_KEY=AppObj.Project_Key
                        PRJ=getPRJ(MetObj,PRJ_KEY,Month)
